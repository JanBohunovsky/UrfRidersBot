...
namespace UrfRidersBot
{
    public class HelpService
    {
        ...
        {
            var notFound = _embedService.CreateError("Could not find specified command.").Build();
            var result = _embedService.CreateBotInfo("Documentation");
            
            // Find the command
            var command = _commandService.Commands.FirstOrDefault(x => x.Aliases.Any(alias => string.Equals(alias, name, StringComparison.InvariantCultureIgnoreCase)));
            if (command == null)
                return notFound;
        ...
        {
            return _commandService.Modules.Where(m => m.Parent == null && m.Commands.Count > 0);
        }
    }
}